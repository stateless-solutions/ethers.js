...
class StatelessProvider extends provider_jsonrpc_js_1.JsonRpcProvider {
    ...
    constructor(url, identities, minimumRequiredAttestations, proverUrl, network, options) {
        ...
        if (proverUrl) {
            this.prover = new provider_jsonrpc_js_1.JsonRpcProvider(proverUrl, network);
        }
    }
    async _send(payload) {
        if (this.prover) {
    ...
    async verifyStatelessProof(params) {
        ...
        // Verify state trie
        const trie = new trie_1.Trie({ root: stateRoot, useKeyHashing: true });
        ...
        // Verify storage trie
        const storageTrie = new trie_1.Trie({ root: storageHash, useKeyHashing: true });
    ...
    async verifyAttestedJsonRpcResponse(response, minimumRequiredAttestations = 1, identities) {
        ...
        for (const [i, attestation] of response.attestations.entries()) {
            ...
            const key = sshpk_1.default.parseKey(sshPublicKey, "ssh");
            if (key.type !== "ed25519") {
    ...
    verifySignature(msgHash, signature, publicKey, hashAlgo) {
        try {
            ...
            return tweetnacl_1.default.sign.detached.verify(msgHashBytes, signatureUint8Array, publicKey);
        }
        catch (error) {
